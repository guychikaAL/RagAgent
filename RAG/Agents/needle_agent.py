...
Extract the fact. Respond with valid JSON only."""
    ...
    def answer(self, question: str, retriever) -> Dict[str, Any]:
        ...
        Args:
            question: User's needle question
        ...
        # Step 3: Extract fact from chunks
        return self.extract(question, retrieved_nodes)
    ...
    def extract(
        ...
        # Extract chunk contents and IDs with hierarchy info
        chunk_contents = []
        chunk_ids = []
        chunk_hierarchy = []  # Track parent-child relationships
        ...
        for node in retrieved_nodes:
            ...
            # Capture hierarchy info for visualization
            chunk_info = {
                'chunk_id': chunk_id,
                'chunk_level': node.node.metadata.get('chunk_level', 'unknown'),
                'parent_id': node.node.metadata.get('parent_id'),
                'score': node.score,
                'text_preview': node.node.text[:200] + "..." if len(node.node.text) > 200 else node.node.text
            }
            chunk_hierarchy.append(chunk_info)
        ...
        return {
            ...
            "mcp_tool_used": False,  # No MCP tool used in this path
            "chunk_hierarchy": chunk_hierarchy  # For GUI visualization
        }
            ...
            if tool_name == 'calculate_days_between':
                ...
                try:
                    ...
                    return {
                        ...
                        "mcp_tool_used": True,
                        "mcp_tool_name": "calculate_days_between",
                        "mcp_tool_details": {
                            "start_date": start,
                            "end_date": end,
                            "result": f"{days} days"
                        },
                        "chunk_hierarchy": []  # MCP tools don't need hierarchy
                    }
